...
namespace Vano.Tools.Azure.RDFE
{
    public static class DefaultAzureCredentialHelper
    {
        ...
        private static readonly ConcurrentDictionary<string, AccessTokenCache> _inMemoryTokenCaches = new ConcurrentDictionary<string, AccessTokenCache>(StringComparer.OrdinalIgnoreCase);
        private static readonly ConcurrentDictionary<string, TokenCredential> _defaultAzureCredentials = new ConcurrentDictionary<string, TokenCredential>(StringComparer.OrdinalIgnoreCase);
        ...
        public static DefaultAzureCredential GetDefaultAzureCredential(string authorityHost, string managedIdentityResourceId = null)
            => (DefaultAzureCredential)_defaultAzureCredentials.GetOrAdd(managedIdentityResourceId ?? string.Empty, id =>
            {
                ...
                if (!isInteractive)
                {
                    ...
                    {
                        options.ManagedIdentityResourceId = new ResourceIdentifier(id);
                    }
                }
        ...
        private static AccessToken GetOrAdd(this ConcurrentDictionary<string, AccessTokenCache> caches, string cacheKey, Func<AccessToken> func)
        {
            ...
            try
            {
                ...
                caches[cacheKey] = new AccessTokenCache() { Token = accessToken, ExpiresOn = GetCacheExpiresOn(accessToken.ExpiresOn) };
                return accessToken;
            }
            catch (Exception)
            {
